...
import time                                         # saeed edit

import logging
log_file = "J:\Saeed Work\May 20\score_log.txt"
logging.basicConfig(
    level=logging.INFO,
...
def load_doc_level_corpus():
    ...
    Returns:
        [str], [str], int -- a tuple of a list of documents, a list of document IDs, and the number of documents
    """
    logging.info("Loading document level corpus.")
    with open(
        Path(global_options.OUTPUT_FOLDER, "scores", "temp", "corpus_doc_level.pickle"),
        "rb",
    ) as in_f:
...
def score_tf(documents, doc_ids, expanded_dict):
    ...
    score.to_csv(
        Path(global_options.OUTPUT_FOLDER, "scores", "scores_TF_DEI_black.csv"), index=False
    )
    ...
    if method == "TF":
        ...
        score.to_csv(
            str(
                Path(
                    ...
                    "scores",
                    "scores_{}_DEI_black.csv".format(method),
                )
            ),
            index=False,
        )
        ...
        pd.DataFrame.from_dict(contribution, orient="index").to_csv(
            Path(
                ...
                "scores",
                "word_contributions",
                "word_contribution_{}_DEI_black.csv".format(method),
            )
        )
...
if __name__ == "__main__":
    ...
    #all_countries = [f.name for f in input_base.iterdir() if f.is_dir() and f.name != "AUT"]
    #all_countries = ['USA']

    for country in all_countries:
        global_options.set_country_paths(country)

        trigram_file = global_options.PROCESSED_FOLDER / "trigram" / "documents.txt"
        sent_id_file = global_options.PROCESSED_FOLDER / "parsed" / "document_sent_ids.txt"
        output_score_dir = global_options.OUTPUT_FOLDER / "scores"
        output_score_dir.mkdir(parents=True, exist_ok=True)

        if not trigram_file.exists() or not sent_id_file.exists():
            print(f"⏭️ Skipping {country}: missing input files")
            continue

        print(f"\n📊 Scoring: {country}")

        #current_dict_path = global_options.OUTPUT_FOLDER / "dict" / "expanded_dict.csv"
        #current_dict_path = Path(r"J:\Saeed Work\May 20\Dictionaries\Exteded_Keywords_Demers_Najah\expanded_dict.csv")
        
        #current_dict_path = Path(r"J:\Saeed Work\May 20\Dictionaries\toxicity_dictionary.csv")
        #current_dict_path = Path(r"J:\Saeed Work\May 20\Dictionaries\SH_dictionary.csv")
        #current_dict_path = Path(r"J:\Saeed Work\May 20\Dictionaries\DEI_JAR.csv")
        #current_dict_path = Path(r"J:\Saeed Work\May 20\Dictionaries\DEI_GD.csv")
        #current_dict_path = Path(r"J:\Saeed Work\May 20\Dictionaries\DEI_Comprehensive.csv")
        current_dict_path = Path(r"J:\Saeed Work\May 20\Dictionaries\DEI_black.csv")

        

        if not current_dict_path.exists():
            print(f"⚠️ Skipping {country}: no dictionary found at {current_dict_path}")
            continue
        ...
        corpus, doc_ids, N_doc = construct_doc_level_corpus(
            sent_corpus_file=trigram_file,
            sent_id_file=sent_id_file
        )
        _ = calculate_df(corpus)

        methods = ["TF", "TFIDF", "WFIDF"]
        for method in methods:
            start_method = time.time()
            logging.info(f"Scoring {country} | Method: {method}")
            score_tf_idf(
                corpus,
                doc_ids,
                N_doc,
            ...
            logging.info(f"⏱ Time for {method} ({country}): {round(time.time() - start_method, 2)} sec")

    logging.info(f"✅ Total run time: {round(time.time() - start_total, 2)} seconds")
