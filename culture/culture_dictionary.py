...
def expand_words_dimension_mean(
    ...
    """
    vocab_number = len(word2vec_model.wv.key_to_index) # Saeed new update
    expanded_words = {}
    all_seeds = set()
    ...
    for dimension in seed_words:
        dimension_words = [
            word for word in seed_words[dimension] if word in word2vec_model.wv.key_to_index # Saeed new update
        ]
...
def rank_by_sim(expanded_words, seed_words, model) -> "dict[str: list]":
    ...
    for dimension in expanded_words.keys():
        dimension_seed_words = [
            word for word in seed_words[dimension] if word in model.wv.key_to_index # Saeed new update
        ]
        similarity_dict = dict()
        for w in expanded_words[dimension]:
            if w in model.wv.key_to_index: # Saeed new update
...
def deduplicate_keywords(word2vec_model, expanded_words, seed_words):
    ...
    for dimension in seed_words:
        for w in seed_words[dimension]:
            if w not in word2vec_model.wv.key_to_index: # Saeed new update
    ...
    for word in list(dup_words):
        ...
        for dimension in expanded_words:
            dimension_seed_words = [
                ...
                if word in word2vec_model.wv.key_to_index # Saeed new update
            ]
